<%@ page contentType="text/html;charset=UTF-8" language="java" isErrorPage="true" %>
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8"/>
  <meta name="viewport" content="width=device-width, initial-scale=1.0"/>
  <title>500 System Error — RERMS</title>
  <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css"/>
  <link rel="stylesheet" href="${pageContext.request.contextPath}/assets/css/style.css"/>
</head>
<body class="auth-page">
<div class="auth-card text-center fade-in-up" style="max-width:480px;">
  <div class="auth-logo" style="background:linear-gradient(135deg,#ef4444,#991b1b);">
    <svg viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2">
      <polygon points="7.86 2 16.14 2 22 7.86 22 16.14 16.14 22 7.86 22 2 16.14 2 7.86 7.86 2"/>
      <line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/>
    </svg>
  </div>
  <h1 style="color:#f8fafc;font-size:2.5rem;font-weight:800;margin-bottom:.5rem;">500</h1>
  <h2 style="color:#f1f5f9;font-size:1.25rem;font-weight:600;margin-bottom:1rem;">Internal Server Error</h2>
  <p style="color:#94a3b8;font-size:.9rem;margin-bottom:2rem;">
    An unexpected error occurred. Please ensure the RMI Server is running and try again.
  </p>
  <a href="${pageContext.request.contextPath}/login" style="display:block;">
    <button class="btn-auth">Return to Login</button>
  </a>
</div>
</body>
</html>
